...
public class Registration {
  ...
  public String selectAllNames(String tableName) {     
    ...
    try {
      ...
      rs = stmt.executeQuery("SELECT lName, fName FROM " + tableName + " ORDER BY " + tableName + "id");
      boolean first = true;
      while (rs.next()) {
        if (!first) sb.append("\n");
        sb.append(rs.getString("lName")).append(", ").append(rs.getString("fName"));
        first = false;
      }
    } 
  ...
  public void deleteCompetitor(String id){
    PreparedStatement pstmt = null;
    try {
      String sql = "DELETE FROM competitor WHERE competitorid = ?";
      pstmt = dbConnect.prepareStatement(sql);
      pstmt.setString(1, id);
      pstmt.executeUpdate();
    } 
